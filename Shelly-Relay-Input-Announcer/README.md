...
- You now need to include the ESPHome config for this functionality in your device's config.

This can be done directly from this repo via a remote package:
...
- Then you need to add a script called `input_change_actions` to put in your desired actions for what happens when the state of the input changes:
...
- OPTIONAL: You can set the default value of the relay MAC address directly in the config via a substitution if you wish. It will remain changeable via a text field inside of Home Assistant or by another method of changing text fields.
...
NB: If you set the default in the config in the optional step mentioned above, then it should already be set to your correct MAC address.

## Debugging and/or more complicated setups
There is also an Event component that will report events of type `init` / `on` / `off`, which can be used for more complicated or specific automations from Home Assistant. It is also a good debugging tool.
